"""Recording *takes*: episodes written to ``recordings/<dataset>/<episode>.rrd``.

Shared between the long-lived local server (``tools/apps/so100_server.py``) and the
standalone CLI recorder (``tools/apps/record_episode.py``). A take is:
...
def begin_take(path: Path, *, episode: str, dataset: str, task: str, proxy_uri: str | None) -> rr.RecordingStream:
    ...
    # The task doubles as the LeRobot task string (read back by export-lerobot).
...
def scan_edits(recordings_dir: Path) -> dict[str, list[Path]]:
    ...
    edits: dict[str, list[Path]] = {}
    if recordings_dir.is_dir():
        for child in sorted(recordings_dir.iterdir()):
            files = sorted((child / "edits").glob("*.rrd")) if child.is_dir() else []
            if files:
